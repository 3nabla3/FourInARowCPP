...
GuiApp::GuiApp(Game &game, uint16_t width, uint16_t height)
    ...
    image.pixels = stbi_load(
            "/home/alban/Documents/coding/FourInARowCPP/icon.jpg", &image.width, &image.height,
            &channels, 4
    );
...
static bool InAlignment(const std::array<Coord, 4> &align, Row row, Col col) {
    return std::ranges::any_of(align.begin(), align.end(), [row, col](const Coord &coord) {
        return coord.first == row && coord.second == col;
    });
}
...
void
GuiApp::KeyCallback(GLFWwindow *window, int key, [[maybe_unused]] int scancode, int action, [[maybe_unused]] int mods) {
